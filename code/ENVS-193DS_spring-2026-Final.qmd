...
One of the predictors is tree height, which is a numerical, continuous variable (measured in cm). As tree height increases, the probability of nest occurrence would increase, as determined by the the logistic model in the results section (paragraph 3). This is likely because height can shield the nests from ground predators, however the nest presence is also dependent on what ant species are present as it can alter the architecture of the tree (ie. the denser canopy when *C. nigriceps* is present), as inferred from the discussion (paragraph 2). 
