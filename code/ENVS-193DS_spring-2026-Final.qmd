...
One of the predictors is tree height, which is a numerical, continuous variable (measured in cm). As tree height increases, the probability of nest occurrence would increase as it would be further away from ground predators and further from the reach of ants (inferred from the introduction and methods). (ASK SOMEONE ABOUT THIS)

The other predictor is acacia ant species, which is a categorical variable (differentiated by type of ant: C. mimosae, C. sjostedti, and C. nigriceps). Because ants are nest predators in acacia trees, it is predicted that the number of nests will be higher in less agressive ant-tree symbionts(C. sjostedti), however, if the acacia ants defend the tree from other nest predators, it is predicted that number of nests will be higher in the more agressive symbionts (C. mimosae and C. nigriceps). This information is found in the third to fourth paragraph in the introduction. 
...
nest_mod2 <- glm(case_control ~ ant_species * height_cm, # "*" = no interaction
                data = nests_clean, # from cleaned data set
                family = "binomial") # binomial data set
...
```
The best model as determined by Akaike’s Information Criterion (AIC) is the model of interaction between tree height and ant species as it has the lower AIC. 
...
```
There is a uniform distirbution of the data as shown by the QQ residuals plot (the data poinst tightly follow the QQ reference line)

On the residuals vs. predicted plots, there are no clear pattern of data points, and they are randomly scattered across plots, thus there are randomly distirbuted residuals. 
...
# generating a prediction object for model 1
preds <- ggpredict(
  nest_mod2,
...
ggplot(data = nests_clean, # data is from cleaned data set 
  ...
  # first layer: geom points (show underlying daya) 
  ...
  labs(x = "Tree Height (cm)", 
       y = "Probaility of bird nest occurence") + 
  # editing theme
...
The predicted probability of nest occurrence at the tallest tree height (600 cm) with *C. sjostedti* present is 0.2 (95% CI: [0.00, 0.99]), with *C. mimosae* present is 0.74 (95% CI: [0.48, 0.90]), and *C. nigriceps* present is 1.00 (95% CI: [1.00, 1.00]). According to this predictions and Figure 1, birds preferred to nest in trees infested with *C. mimosae* and *C. nigriceps*, with more nesting on the taller trees with *C. nigriceps*, however height did not seem to influence the nest presence on trees infested with *C. mimosae*. Birds also rarely nested on trees with *C. sjostedti* present, regardless of height.

This selection towards trees inhabited by more aggressive ant species (*C. mimosae* and *C. nigriceps*) as they do not directly attck the nestlings or adult birds and may reduce the risk of predation by vertebrate and invertebrate herbivores. The specific preference toward *C. nigriceps* out of the two aggressive species is that this species is that its presence leads to denser canopies by altering tree shoot structure, which further protects the nests from other predators such as raptors or snakes. 
