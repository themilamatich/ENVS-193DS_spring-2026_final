...
Here is the link to my [GitHub repository.](https://github.com/themilamatich/ENVS-193DS_spring-2026_final)
...
Habitat distance to water edge significantly influenced the likelihood of whether or not the shorebird American Avocet used the microhabitat.
...
One additional variable that could influence the probability of American Avocet microhabitat use could be seasonality measured across a long time series (2-3 years) and keeping track of date (categorical variable) that would be categorically placed into Fall, Spring, Summer or Winter. This factor would influence the habitat use as it influences shorebird migration patterns, which determines whether or not the birds are in the location of study, and therefor keeping track of this data alongside the known migration pattern/season of each species will help explain why birds may be present (or not present) at different times.
...
The 1s and the 0s in a biological context represents a binary outcome of presence (1) or not present (0). In this specific study, a 1 represents a tree with a bird nest and 0 represents a tree with no bird nests. 

## c. Purpose of study 

One of the predictors is tree height, which is a numerical, continuous variable (measured in cm). As tree height increases, the probability of nest occurrence would increase, as determined by the the logistic model in the results section (paragraph 2), but it may also have no effect on nest occurrence dependent on what ant species are present (results section, paragraph 3). This is likely because height can shield the nests from ground predators, however the nest presence is also dependent on what ant species are present as it can alter the architecture of the tree (ie. the denser canopy when *C. nigriceps* is present), as inferred from the discussion (paragraph 2). 
...
# running diagnostic plots using simulated residuals from DHARMa package 
plot(
...
ggplot(data = nests_clean, # data is from cleaned data set 
  ...
  geom_ribbon(data = preds, # data from model predictions
              mapping = aes(x = height_cm, # x-axis: tree height
                            y = predicted, # y-axis: predicted probability
                            ymin = conf.low, # 95% CI (min)
                            ymax = conf.high, # 95% CI (max)
...
**Figure 1. Probability of bird nest occurrence differs based on the present ant species and tree height.** Figures are separated by ant species and this is differentiated through distinctive panels and colors, where *C. sjostedti* is rose-brown, *C. mimosae* is yellow, and *C. nigriceps* is red. Open circle points represent individual observations of nest occurrence in each scenario, the line represents model predictions of nest occurrence given tree height (cm) and ant species, and the ribbon represents the 95% confidence interval. Data originally from Lujan, Ema et al. 2023. “Symbiotic acacia ants drive nesting behavior by birds in an African savanna.” Biotropica 55:6. doi: 10.1111/btp.13276.
...
- My affective visualization differed greatly from my exploratory visualization in Homework 2 as it represented my data in a different way. Instead of using a linear model or a box plot, I use a bar graph diagram in order to show my step count on each given day. It also shows how my data fluctuates through time as each bar represents one day, while the visualizations in homework 2 do not differentiate each day on a linear time scale (they are just represented by points). Additionally, instead of continuing to explore temperature as a predictor variable, I chose to do amount of homework as I felt like it would be easiest to visualize given it being a categorical variable I could assign colors to. 
...
An analysis I could run to answer this question would be Two-Sample T-test, however the specific type of test depends on whether my data is normal and variances are equal (Student's if variances are equal and data is normally distributed, Welch's if variances are not equal but data is normally distributed, and Mann Whitney U if data are not normally distributed). This test makes sense as it will compare my average (or median) step count across the weekdays and the weekends, which are two groups of data. It also make sense because my predictor variable is categorical, and I want to compare step count across two groups (weekday/weekend) and see how they differ. 
...
# running a Student's t-test due to results from normality & variance test 
t.test(
  # formula: response variable ~ grouping variable
  step_count ~ weekday_or_weekend, 
...
# creating a new object called personal_summary to calculate values
...
### Generating Jitter Plot for Comparing Means
```{r}
#| label: making-a-jitter 
# base layer: ggplot
ggplot(data = personal_clean, # data = cleaned data set 
  ...
  geom_jitter(width = 0.1, # narrow jitter width
              ...
              alpha = 0.75, # transparent points to be visually distinct
  ...
  geom_errorbar(data = personal_summary, # data from summary
                mapping = aes(x = weekday_or_weekend, # x-axis = weekday/end
                              y = mean, # y-axis = mean
  ...
  # changing theme elements to differ from default
...
## e. Write a caption 
**Figure 2. Number of steps taken differs on weekday versus weekend.** Small open-circular points represent the total number of steps recorded on a given weekday (yellow points) or weekend (blue points) from 2026-04-21 to 2026-05-30. The average step count for each weekday (yellow) and weekend (blue) is shown by the solid dot, while the error bars represent the standard deviation of the data to show the variability within the weekdays or weekends. Data collected by Mila Matich from 2026-04-21 to 2026-05-30.

## f. Write about your results 
My results suggest that, on average, my step count was greater on weekends (n = 11, mean = 13,112 steps) than weekdays (n = 29, mean = 9,507 steps). I found a large negative effect (Cohen's d = -0.927) of day-type on mean step count between the weekdays and weekends (Student's t-test, t(38) = -2.8381, p = 0.007245, $\alpha$ = 0.05). 
